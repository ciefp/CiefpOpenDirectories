#!/bin/bash
##setup command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/ciefp/CiefpOpenDirectories/main/installer.sh -O - | /bin/sh
...
TMPPATH=/tmp/CiefpOpenDirectories

if [ ! -d /usr/lib64 ]; then
	PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/CiefpOpenDirectories
else
	PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/CiefpOpenDirectories
fi
...
if [ -f /var/lib/dpkg/status ]; then
   ...
   wget https://github.com/ciefp/CiefpOpenDirectories/archive/refs/heads/main.tar.gz
   tar -xzf main.tar.gz
   cp -r 'CiefpOpenDirectories-main/usr' '/'
set +e
cd
sleep 2
...
echo ""
echo "#########################################################"
echo "#        CiefpOpenDirectories INSTALLED SUCCESSFULLY    #"
